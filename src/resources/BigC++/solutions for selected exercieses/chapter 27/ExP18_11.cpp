...
const int ID_LEVEL = 1000;
const int ID_GUESS = 1001;
const int ID_PLAYER_NAME = 1002;
const int ID_PLAYER_LEVEL = 1003;
...
public:
   ...
   */
   void update_level();
   /**
      Updates the player's level from the text control.
      @param event the event descriptor
   */         
...
BEGIN_EVENT_TABLE(GameFrame, wxFrame)
   EVT_BUTTON(ID_LEVEL, GameFrame::OnLevel)
   ...
   : wxFrame(NULL, -1, "GameFrame")
{
   ...
   wxButton* level_button = new wxButton(this, 
      ID_LEVEL, "Set");
   ...
   bottom_sizer->Add(new wxStaticText(this, -1, "Level"));
   bottom_sizer->Add(level_text);
   bottom_sizer->Add(level_button);
...
void GameFrame::update_level()
{
   char buf[10];
   sprintf(buf, "%d", player.get_level());
   level_text->SetValue(buf);
}

void GameFrame::OnLevel(wxCommandEvent& event)
{
   tries++;
   int level = atoi(level_text->GetValue().c_str());
   if (1 <= level && level <= 4)

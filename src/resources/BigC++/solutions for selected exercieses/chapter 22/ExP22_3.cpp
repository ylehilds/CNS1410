...
void merge(vector<T>& a, int from, int mid, int to, CMP comp)
{  
...
*/
template <typename T, typename CMP>
void merge_sort(vector<T>& a, int from, int to, CMP comp)
{  
   if (from == to) return;
   int mid = (from + to) / 2;
   /* sort the first and the second half */
   merge_sort(a, from, mid, comp);
   merge_sort(a, mid + 1, to, comp);
...
{  
   ...
   for (int i = 0; i < v.size(); i++)
      v[i] = rand_int(1, 100);
   print(v);
   merge_sort(v, 0, v.size() - 1, less<int>());
   print(v);
   return 0;
}

...
/*void BasicPanel::OnLoad(wxCommandEvent &event)
{
    wxString anEmployee;
    int size = employees.size( );
    for (int i = 0; i < size; i++)
    {
        anEmployee = employees[ i ]->getName( ).c_str( );
...
void BasicPanel::OnLoad(wxCommandEvent &event)
 ...
 wxString textCtrlData;
    int selectionIndex = employeeListBox->GetSelection( );
     ...
     textCtrlData = employees[ selectionIndex ]->getName( ).c_str( );
    nameTextCtrl->Load( );
    (*nameTextCtrl) << textCtrlData;
...
// Add Text Controls
 ...
 wxStaticText* stext7 = new wxStaticText(aPanel,wxID_ANY,"Comm Rate:",
                                        wxPoint(60,380));
employeeListBox = new wxListBox(aPanel, wxID_ANY, wxPoint(400,30), 
        wxSize(180, 375), 0, NULL, wxLB_SINGLE | wxLB_NEEDED_SB); 

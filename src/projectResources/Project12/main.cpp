...
{
	int selectionIndexSales = employeeListBox->GetSelection( );
	double sale;
	ifstream salefile;
	...
	{
		int typesale;
		//salefile >> typesale >> sale;
		if (!salefile)// data validation, to check for any bad data
		{
			cout << "end of file reached, error";
			break;
		}
	
		E3.push_back(pointer1);
		//s->setSalesAmount(salesAmount);
		
		//pointer1->readData(salefile);
		//E3.push_back(pointer1);
	}

	(*text6) << E3[selectionIndexSales]->get_commission();//->get_phoneNumber().c_str( );
		text6->Clear( ); 
	
	//(*text6) << E3[0];//->get_phoneNumber().c_str( );
	//text3->Clear( ); 
...
{  
	...
	text->Clear( ); 
	/*if (selectionIndex == 4 | selectionIndex == 5 | selectionIndex == 6 )
	{
		//Sales* s = dynamic_cast<Sales*>(p);
		text7->Clear( ); 
		(*text7) << E1[selectionIndex]->get_socialSecurityNumber().c_str( );
	}
	else 
	{
		text7->Clear( ); 
...
//		employeeListBox = new wxListBox(this, LIST_SELECT, wxPoint(280,70), 
 //       wxSize(180, 250), 0, NULL, wxLB_SINGLE | wxLB_NEEDED_SB); 
